...
# scripts/make_bathymetry.jl
#!/usr/bin/env julia

using Oceananigans
using Oceananigans.Units: hours, days
using Oceananigans.OutputWriters: Checkpointer, TimeInterval
using Oceananigans.BoundaryConditions: ValueBoundaryCondition, FluxBoundaryCondition
using Printf
...
# EOS API for this Oceananigans version
...
# ------------------------------
# Lateral BCs (free-slip/no-normal-flow)
# ------------------------------
# u is at (Face, Center, Center) → west/east need ValueBC (normal), north/south can use FluxBC (tangential)
u_bcs = FieldBoundaryConditions(
    west   = ValueBoundaryCondition(0.0),
    east   = ValueBoundaryCondition(0.0),
    south  = FluxBoundaryCondition(0.0),
    north  = FluxBoundaryCondition(0.0),
    bottom = FluxBoundaryCondition(0.0),
    top    = FluxBoundaryCondition(0.0),
)

# v is at (Center, Face, Center) → south/north need ValueBC (normal), west/east can use FluxBC (tangential)
v_bcs = FieldBoundaryConditions(
    west   = FluxBoundaryCondition(0.0),
    east   = FluxBoundaryCondition(0.0),
...
# Tracers at cell centers → zero flux on all sides
T_bcs = FieldBoundaryConditions(
    west = FluxBoundaryCondition(0.0), east = FluxBoundaryCondition(0.0),
    south = FluxBoundaryCondition(0.0), north = FluxBoundaryCondition(0.0),
    top = FluxBoundaryCondition(0.0),   bottom = FluxBoundaryCondition(0.0),
)
